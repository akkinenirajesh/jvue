...
public class Pass1 {
     ...
     * default two 8 bit operants are widened to two 16 bit aperants
     * accordingly.
     */
    ...
    public Pass1(JavaClass jc, J2JSCompiler compiler) {
        this.compiler = compiler;
        constantPool = jc.getConstantPool();
    }

    private CatchClause createCatchClause(TryStatement tryStmt,
            ExceptionHandler handle) {
        CatchClause cStmt = new CatchClause(handle.getHandlerPC());
        VariableDeclaration decl = new VariableDeclaration(
                VariableDeclaration.LOCAL_PARAMETER);
        decl.setName("_EX_");
    ...
    private void makeTryFrame(TryStatement stmt) {
        ...
        CatchClause clause = (CatchClause) stmt.getCatchStatements()
                .getFirstChild();
        while (clause != null) {
            // Push implicit exception.
            Node catchNode = graph.createNode(clause.getBeginIndex());
            // catchNode.type = NodeType.CATCH;
            catchNode.stack = new ASTNodeStack(
                    new VariableBinding(clause.getException()));
    ...
    private void compileCodeException() {
        ...
        ExceptionHandler handle = handleIterator.hasNext()
                ? handleIterator.next() : null;
        while (handle != null) {
            ...
            tryStmt.header = (TryHeaderNode) graph
                    .createNode(TryHeaderNode.class);
            ...
            // handler.
            while (handle != null && !handle.isDefault()
                    && handle.getStartPC() == start
                    && handle.getEndPC() == end) {
                if (cStmt != null) {
                    cStmt.setEndIndex(handle.getHandlerPC() - 1);
                }
                cStmt = createCatchClause(tryStmt, handle);
                handle = handleIterator.hasNext() ? handleIterator.next()
                        : null;
            }

            int foo = -1;
            if (handle != null && handle.isDefault()
                    && handle.getStartPC() == start) {
            ...
            if (catches.getChildCount() == 0) {
                throw new ParseException(
                        "A try clause must have at least one (possibly default) catch clause",
                        tryStmt);
            }
        ...
        for (InstructionHandle handle : handles) {
            ...
            if (targeters != null) {
                for (InstructionTargeter targeter : handle.getTargeters()) {
                    System.out.println("    Targeter: " + targeter.toString()
                            + " " + targeter.getClass());
                }
            }
        }
    ...
    public void parse(Method theMethod, MethodDeclaration theMethodDecl)
            throws IOException {
        method = theMethod;
    ...
    private void expressionsToVariables(Node node, boolean clone) {
        ...
        for (int i = 0; i < node.stack.size(); i++) {
            ...
            VariableBinding vb = methodDecl.createAnonymousVariableBinding(
                    expr.getTypeBinding(), true);
    ...
    /**
     * Merges all source stacks into one target stack. If a layer over all
     * stacks contains the identical element, then this element is propagated.
     * For example, if there are two stacks [a, b, c] and [d, b, e], we append
     * to the source nodes {t1=a; t2=c} and {t1=d; t2=e}, and populate the
     * specified target stack with [t1, b, t2].
     */
    private void mergeStacks(Collection sources, ASTNodeStack target) {
        ...
        for (int index = 0; index < stackSize; index++) {
            ...
            if (obj instanceof Expression) {
                ...
                VariableBinding vb = methodDecl
                        .createAnonymousVariableBinding((Type) obj, true);
        ...
        while (bytes.available() > 0) {
            ...
            if (stmt instanceof JumpSubRoutine) {
                ...
                if (finallyNode == null) {
                    ...
                    // Generate dummy expression for the astore instruction of
                    // the finally block.
                ...
                if (cNode.preds().size() == 1 && finallyNode.preds().size() == 0
                    ...
                    // TODO: This only works if default handler is a single
                    // node!!
                    TryHeaderNode tryHeaderNode = (TryHeaderNode) cNode
                            .getPred();
                ...
                if (bytes.getIndex() == cond.getTargetIndex()) {
                    // This is a conditional branch with an empty body, i.e. not
                    // a branch at all. We ignore it.
                } else {
                    Node elseNode = graph.getOrCreateNode(bytes.getIndex());

                    Node ifNode;
                    if (cond.getTargetIndex() <= pc) {
                        Node[] nodes = graph.getOrSplitNodeAt(cNode,
                                cond.getTargetIndex());
                        cNode = nodes[0];
                        ifNode = nodes[1];
                    } else {
    ...
    public static void dump(J2JSCompiler compiler, ASTNode node, String msg) {
        if (!logger.isDebugEnabled())
            return;
        ...
        out.println(msg);
        SimpleGenerator generator = new SimpleGenerator(out, compiler);
    ...
    private VariableBinding createVariableBinding(int slot, Type type,
            boolean isWrite) {
        return methodDecl.createVariableBinding(VariableDeclaration
                .getLocalVariableName(method, slot, bytes.getIndex()), type,
                isWrite);
    }

    private InfixExpression createInfixRightLeft(InfixExpression.Operator op,
            Expression right, Expression left, Type type) {
    ...
    private PrefixExpression createPrefix(PrefixExpression.Operator op,
            ASTNode operand, Type type) {
    ...
    private Expression[] duplicate(Expression e) {
        ...
        VariableBinding vb1 = methodDecl
                .createAnonymousVariableBinding(e.getTypeBinding(), true);
        VariableBinding vb2 = (VariableBinding) vb1.clone();
     ...
     */
    private SwitchEdge getOrCreateCaseGroup(Node header,
            Map<Integer, SwitchEdge> switchEdges, int startPc) {
        SwitchEdge switchEdge = switchEdges.get(startPc);
        if (switchEdge == null) {
            Node caseGroupNode = graph.createNode(startPc);
            switchEdge = (SwitchEdge) graph.addEdge(header, caseGroupNode,
                    SwitchEdge.class);
    ...
    private ASTNode parseInstruction() throws IOException {
        ...
        logger.debug(currentIndex + " " + instructionType.getName() + "["
                + opcode + "] ");
        ...
        case Const.TABLESWITCH:
            // Format: tableswitch, padding(byte), padding(byte), padding(byte),
            // default(int), low(int), high(int)
            // Operand stack: ..., index(int) -> ...
        case Const.LOOKUPSWITCH: {
            // Format: lookupswitch, padding(byte), padding(byte),
            // padding(byte), default(int), npairs(int)
            // Operand stack: ..., key() -> ...
            ...
            // Skip (0-3) padding bytes, i.e., the following bytes are
            // 4-byte-aligned.
            for (int i = 0; i < noPadBytes; i++) {
                byte b;

                if ((b = bytes.readByte()) != 0) {
                    logger.warn("Padding byte != 0 in "
                            + instructionType.getName() + ":" + b);
                }
            }
            ...
            for (int i = 0; i < npairs; i++) {
                ...
                SwitchEdge switchEdge = getOrCreateCaseGroup(switchNode,
                        caseGroups, offset + bytes.readInt());
                switchEdge.expressions
                        .add(NumberLiteral.create(new Integer(key)));
            }
        ...
        case Const.CHECKCAST: {
            ...
            ObjectType type = new ObjectType(
                    c.getBytes(constantPool).replace('/', '.'));
        ...
        case Const.IFEQ:
            ...
            // Operand stack: ..., value() -> ...
            instruction = createConditional(currentIndex,
                    InfixExpression.Operator.EQUALS, NumberLiteral.create(0));
            break;
        case Const.IFNE:
        ...
        case Const.IFGT:
            ...
            // Operand stack: ..., value() -> ...
            instruction = createConditional(currentIndex,
                    InfixExpression.Operator.GREATER, NumberLiteral.create(0));
            break;
        case Const.IFLE:
        ...
        case Const.IFLT:
            ...
            // Operand stack: ..., value() -> ...
            instruction = createConditional(currentIndex,
                    InfixExpression.Operator.LESS, NumberLiteral.create(0));
            break;
        case Const.IFNONNULL:
            // Format: ifnonnull, branch(short)
            // Operand stack: ..., value() -> ...
            instruction = createConditional(currentIndex,
                    InfixExpression.Operator.NOT_EQUALS, new NullLiteral());
            break;
        case Const.IFNULL:
            // Format: ifnull, branch(short)
            // Operand stack: ..., value() -> ...
            instruction = createConditional(currentIndex,
                    InfixExpression.Operator.EQUALS, new NullLiteral());
            break;
        ...
        case Const.DCMPG: {
            // Format: dcmpg
            // Operand stack: ..., value1(), value2() -> ..., result(int)
            MethodBinding binding = MethodBinding.lookup("javascript.Utils",
                    "cmp", "(DDI)I");
        ...
        case Const.NEW: {
            ...
            ConstantClass c = (ConstantClass) constantPool
                    .getConstant(bytes.readUnsignedShort());
            ObjectType type = new ObjectType(
                    c.getBytes(constantPool).replace('/', '.'));
        ...
        case Const.NEWARRAY: {
            ...
            String componentSignature = BasicType.getType(bytes.readByte())
                    .getSignature();
            ...
            ArrayCreation ac = new ArrayCreation(methodDecl,
                    new ObjectType("[" + componentSignature), dimensions);
            instruction = ac;
            break;
        }
        ...
        case Const.ANEWARRAY: {
            ...
            List<ASTNode> dimensions = new ArrayList<ASTNode>();
            dimensions.add(stack.pop());
            ArrayCreation ac = new ArrayCreation(methodDecl, arrayType,
                    dimensions);
            instruction = ac;
            break;
        }
        ...
        case Const.MULTIANEWARRAY: {
            ...
            // Operand stack: ..., count1(), ...(), countN() -> ...,
            // arrayref(object)

            ConstantClass c = (ConstantClass) constantPool
                    .getConstant(bytes.readUnsignedShort());
            ObjectType arrayType = new ObjectType(
                    c.getBytes(constantPool).replace('/', '.'));

            // Create a new multidimensional array of the array type arrayType.
            // The array type must be an array class type of dimensionality
        ...
        case Const.PUTFIELD: {
            ...
            int index = bytes.readUnsignedShort();
            ConstantFieldref fieldRef = (ConstantFieldref) constantPool
                    .getConstant(index, Constants.CONSTANT_Fieldref);
        ...
        case Const.DUP2:
            // (0) Format: dup2
            // (0) Operand stack: ..., value2(cat1), value1(cat1) -> ...,
            // value2(cat1), value1(cat1), value2(cat1), value1(cat1)
            // (1) Format: dup2
            // (1) Operand stack: ..., value(cat2) -> ..., value(cat2),
            // value(cat2)
            if (form.getIndex() == 0) {
                dup2();
                instruction = stack.pop();
        ...
        case Const.DUP_X1: {
            // Format: dup_x1
            // Operand stack: ..., value2(cat1), value1(cat1) -> ...,
            // value1(cat1), value2(cat1), value1(cat1)
            dup1();
            stack.rotate(2);
            instruction = stack.pop();
            break;
        ...
        case Const.DUP_X2: {
            // (0) Format: dup_x2
            // (0) Operand stack: ..., value3(cat1), value2(cat1), value1(cat1)
            // -> ..., value1(cat1), value3(cat1), value2(cat1), value1(cat1)
            // (1) Format: dup_x2
            // (1) Operand stack: ..., value2(cat2), value1(cat1) -> ...,
            // value1(cat1), value2(cat2), value1(cat1)
            if (form.getIndex() == 0) {
                dup1();
                stack.rotate(3);
            } else {
        ...
        case Const.DUP2_X1:
            // (0) Format: dup2_x1
            // (0) Operand stack: ..., value3(cat1), value2(cat1), value1(cat1)
            // -> ..., value2(cat1), value1(cat1), value3(cat1), value2(cat1),
            // value1(cat1)
            // (1) Format: dup2_x1
            // (1) Operand stack: ..., value2(cat1), value1(cat2) -> ...,
            // value1(cat2), value2(cat1), value1(cat2)
            if (form.getIndex() == 0) {
                dup2();
                stack.rotate(4);
                stack.rotate(4);
        ...
        case Const.DUP2_X2:
            // (0) Format: dup2_x2
            // (0) Operand stack: ..., value4(cat1), value3(cat1), value2(cat1),
            // value1(cat1) -> ..., value2(cat1), value1(cat1), value4(cat1),
            // value3(cat1), value2(cat1), value1(cat1)
            // (1) Format: dup2_x2
            // (1) Operand stack: ..., value3(cat1), value2(cat1), value1(cat2)
            // -> ..., value1(cat2), value3(cat1), value2(cat1), value1(cat2)
            // (2) Format: dup2_x2
            // (2) Operand stack: ..., value3(cat2), value2(cat1), value1(cat1)
            // -> ..., value2(cat1), value1(cat1), value3(cat2), value2(cat1),
            // value1(cat1)
            // (3) Format: dup2_x2
            // (3) Operand stack: ..., value2(cat2), value1(cat2) -> ...,
            // value1(cat2), value2(cat2), value1(cat2)
            if (form.getIndex() == 0) {
                dup2();
                stack.rotate(5);
                stack.rotate(5);
        ...
        case Const.I2C:
            // Format: i2c
            // Operand stack: ..., value() -> ..., result(byte)
            // Operation i2f may result in a loss of precision because values of
            // type float have only 24 significand bits.
            // Operation l2d may lose precision because values of type double
            // have only 53 significand bits
            instruction = new PrimitiveCast(opcode, stack.pop(),
                    form.getResultType());
            break;
        ...
        case Const.F2D:
            // Format: f2d
            // Operand stack: ..., value() -> ..., result(double)
            // Widening preserves the numeric value exactly, see VM Spec2,
            // Section 2.6.2.
            // For operation f2d there may be a rounding related to value set
            // conversion.
        ...
        case Const.DNEG:
            // Format: dneg
            // Operand stack: ..., value() -> ..., result(double)
            instruction = createPrefix(PrefixExpression.MINUS, stack.pop(),
                    form.getResultType());
            break;
        ...
        case Const.LSHR:
            ...
            instruction = createInfixRightLeft(
                    InfixExpression.Operator.RIGHT_SHIFT_SIGNED, stack.pop(),
                    stack.pop(), form.getResultType());
            break;
        ...
        case Const.LSHL:
            ...
            // Operand stack: ..., value1(), value2() -> ..., result(long)
            instruction = createInfixRightLeft(
                    InfixExpression.Operator.LEFT_SHIFT, stack.pop(),
                    stack.pop(), form.getResultType());
            break;
        ...
        case Const.LUSHR:
            ...
            // Operand stack: ..., value1(), value2() -> ..., result(long)
            instruction = createInfixRightLeft(
                    InfixExpression.Operator.RIGHT_SHIFT_UNSIGNED, stack.pop(),
                    stack.pop(), form.getResultType());
            break;
        ...
        case Const.DADD:
            // Format: dadd
            // Operand stack: ..., value1(double), value2(double) -> ...,
            // result(double)
            instruction = createInfixRightLeft(InfixExpression.Operator.PLUS,
                    stack.pop(), stack.pop(), form.getResultType());
            break;
        ...
        case Const.DSUB:
            ...
            // Operand stack: ..., value1(double), value2(double) -> ...,
            // result(double)
            instruction = createInfixRightLeft(InfixExpression.Operator.MINUS,
                    stack.pop(), stack.pop(), form.getResultType());
            break;
        ...
        case Const.DMUL:
            ...
            instruction = createInfixRightLeft(InfixExpression.Operator.TIMES,
                    stack.pop(), stack.pop(), form.getResultType());
            break;
        ...
        case Const.DDIV:
            // Format: ddiv
            // Operand stack: ..., value1(), value2() -> ..., result(double)
            instruction = createInfixRightLeft(InfixExpression.Operator.DIVIDE,
                    stack.pop(), stack.pop(), form.getResultType());
            break;
        ...
        case Const.DREM:
            ...
            // Operand stack: ..., value1(), value2() -> ..., result(double)
            instruction = createInfixRightLeft(
                    InfixExpression.Operator.REMAINDER, stack.pop(),
                    stack.pop(), form.getResultType());
            break;
        ...
        case Const.LXOR:
            // Format: lxor
            // Operand stack: ..., value1(), value2() -> ..., result(long)
            instruction = createInfixRightLeft(InfixExpression.Operator.XOR,
                    stack.pop(), stack.pop(), form.getResultType());
            break;
        ...
        case Const.LAND:
            // Format: land
            // Operand stack: ..., value1(), value2() -> ..., result(long)
            instruction = createInfixRightLeft(InfixExpression.Operator.AND,
                    stack.pop(), stack.pop(), form.getResultType());
            break;
        ...
        case Const.LOR:
            // Format: lor
            // Operand stack: ..., value1(), value2() -> ..., result(long)
            instruction = createInfixRightLeft(InfixExpression.Operator.OR,
                    stack.pop(), stack.pop(), form.getResultType());
            break;
        ...
        case Const.IINC: {
            ...
            VariableBinding reference = createVariableBinding(index, Type.INT,
                    true);
            ...
            assign.setRightHandSide(
                    NumberLiteral.create(new Integer(constByte)));
            instruction = assign;
            break;
        }
        ...
        case Const.ILOAD_3: {
            // Format: iload_3
            // Operand stack: ... -> ..., value(int)
            VariableBinding reference = createVariableBinding(
                    opcode - Const.ILOAD_0, Type.INT, false);
            reference.setField(false);
        ...
        case Const.LLOAD_3: {
            ...
            // Operand stack: ... -> ..., value(long)
            VariableBinding reference = createVariableBinding(
                    opcode - Const.LLOAD_0, Type.LONG, false);
            reference.setField(false);
            instruction = reference;
            break;
        }
        ...
        case Const.FLOAD_3: {
            ...
            // Operand stack: ... -> ..., value(float)
            VariableBinding reference = createVariableBinding(
                    opcode - Const.FLOAD_0, Type.FLOAT, false);
            reference.setField(false);
            instruction = reference;
            break;
        }
        ...
        case Const.DLOAD_3: {
            ...
            // Operand stack: ... -> ..., value(double)
            VariableBinding reference = createVariableBinding(
                    opcode - Const.DLOAD_0, Type.DOUBLE, false);
            reference.setField(false);
            instruction = reference;
            break;
        }
        ...
        case Const.ALOAD_3: {
            ...
            if (opcode == Const.ALOAD_0
                    && !Modifier.isStatic(methodDecl.getAccess())) {
                ThisExpression reference = new ThisExpression();
                instruction = reference;
            } else {
                VariableBinding reference = createVariableBinding(
                        opcode - Const.ALOAD_0, Type.OBJECT, false);
        ...
        case Const.DLOAD: {
            ...
            // Operand stack: ... -> ..., value(double)
            VariableBinding reference = createVariableBinding(readUnsigned(),
                    form.getResultType(), false);
            reference.setField(false);
            instruction = reference;
            break;
        }
        ...
        case Const.DSTORE_3: {
            ...
            Assignment a = new Assignment(Assignment.Operator.ASSIGN);
            VariableBinding reference = createVariableBinding(index,
                    Type.DOUBLE, true);
            reference.setField(false);
        ...
        case Const.FSTORE_3: {
            ...
            Assignment a = new Assignment(Assignment.Operator.ASSIGN);
            VariableBinding reference = createVariableBinding(index, Type.FLOAT,
                    true);
            reference.setField(false);
            a.setLeftHandSide(reference);
            a.setRightHandSide(stack.pop());
        ...
        case Const.LSTORE_3: {
            ...
            Assignment a = new Assignment(Assignment.Operator.ASSIGN);
            VariableBinding reference = createVariableBinding(index, Type.LONG,
                    true);
            reference.setField(false);
            a.setLeftHandSide(reference);
            a.setRightHandSide(stack.pop());
        ...
        case Const.ASTORE_3: {
            ...
            // It will store the implied exception. This exception is pushed
            // onto the
            // stack by makeTryFrames(). However, if the handler is unreachable,
            // the
            // Java Compiler may not generate an exception handler, and the
            // stack is therefore
            // empty.
            // TODO: Refactor this Class by using
        ...
        case Const.ICONST_5:
            // Format: iconst_5
            // Operand stack: ... -> ..., i(int)
            instruction = NumberLiteral
                    .create(new Integer(-1 + opcode - Const.ICONST_M1));
            break;
        ...
        case Const.LCONST_1:
            ...
            instruction = NumberLiteral
                    .create(new Long(opcode - Const.LCONST_0));
            break;
        ...
        case Const.FCONST_2:
            ...
            instruction = NumberLiteral
                    .create(new Float(opcode - Const.FCONST_0));
            break;
        ...
        case Const.DCONST_1:
            ...
            instruction = NumberLiteral
                    .create(new Double(opcode - Const.DCONST_0));
            break;
        ...
        case Const.BIPUSH: {
            ...
            NumberLiteral literal = NumberLiteral
                    .create(new Byte(bytes.readByte()));
            instruction = literal;
            break;
        }
        ...
        case Const.SIPUSH: {
            ...
            NumberLiteral il = NumberLiteral
                    .create(new Short(bytes.readShort()));
            instruction = il;
            break;
        }
        ...
        case Const.LDC2_W: {
            ...
            if (opcode == Const.LDC2_W
                    && (constant.getTag() != Constants.CONSTANT_Double
                            && constant.getTag() != Constants.CONSTANT_Long))
                throw new RuntimeException("LDC2_W must load long or double");

            if (constant.getTag() == Constants.CONSTANT_Integer) {
                instruction = NumberLiteral.create(
                        new Integer(((ConstantInteger) constant).getBytes()));
            } else if (constant.getTag() == Constants.CONSTANT_Float) {
                instruction = NumberLiteral.create(
                        new Float(((ConstantFloat) constant).getBytes()));
            } else if (constant.getTag() == Constants.CONSTANT_Long) {
                instruction = NumberLiteral
                        .create(new Long(((ConstantLong) constant).getBytes()));
            } else if (constant.getTag() == Constants.CONSTANT_Double) {
                instruction = NumberLiteral.create(
                        new Double(((ConstantDouble) constant).getBytes()));
            } else if (constant.getTag() == Constants.CONSTANT_Utf8) {
                instruction = new StringLiteral(
                ...
                Signature signature = Project.getSingleton().getSignature(
                        ((ConstantClass) constant).getBytes(constantPool));
                instruction = new ClassLiteral(signature);
            } else {
                throw new RuntimeException(
                        "Cannot handle constant tag: " + constant.getTag());
            }
            break;
        }
        ...
        case Const.POP2: {
            ...
            if (opcode == Const.POP2 && form.getIndex() == 1) {
                throw new UnsupportedOperationException("InstructionType "
                        + instructionType.getName() + " not supported");
            }
            ...
            if (!(a instanceof VariableBinding)) {
                // Can't have binding on stack! TODO: VariableBinding must not
                // be an ASTNode,
        ...
        case Const.INVOKESTATIC: {
            ...
            int index = bytes.readUnsignedShort();
            MethodBinding methodBinding = MethodBinding.lookup(index,
                    constantPool);
    ...
    ConditionalBranch createConditional(int currentIndex,
            InfixExpression.Operator operator) throws IOException {
        ConditionalBranch c = new ConditionalBranch(
                currentIndex + bytes.readShort());
    ...
    ConditionalBranch createConditional(int currentIndex,
            InfixExpression.Operator operator, Expression rightOperand)
            throws IOException {
        ConditionalBranch c = new ConditionalBranch(
                currentIndex + bytes.readShort());
        Expression leftOperand = stack.pop();

        if (leftOperand.getTypeBinding() != null
                && leftOperand.getTypeBinding() == Type.BOOLEAN) {
            if (operator == InfixExpression.Operator.EQUALS
                    && NumberLiteral.isZero(rightOperand)) {
    ...
    public static String constantToString(Constant c, ConstantPool constantPool)
        ...
        case Constants.CONSTANT_Class:
            str = Utility.compactClassName(
                    ((ConstantClass) c).getBytes(constantPool), false);
            break;

        case Constants.CONSTANT_String:
            str = "\""
                    + Utils.escape(((ConstantString) c).getBytes(constantPool))
                    + "\"";
            break;

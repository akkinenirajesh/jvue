...
public class Optimizer {
	...
	public static Expression simplifyBooleanExpression(Expression expr, boolean negate) {
		...
		if (expr instanceof InfixExpression && expr.getTypeBinding() == Type.BOOLEAN) {
			...
			out.widen(in);
			out.setOperands(simplifyBooleanExpression(in.getLeftOperand(), negate),
					simplifyBooleanExpression(in.getRightOperand(), negate));
			return out;
		}
	...
	/**
	 * Reduces occurences of temp = x; y = temp; to y = x;
	 */
	...
	/**
	 * If the specified expression corresponds to xy + 1 or xy - (-1), returns
	 * the INCREMENT operator. If the specified expression corresponds to xy - 1
	 * or xy + (-1), returns the DECREMENT operator. Otherwise, null is
	 * returned.
	 * 
	 * @param expr
	 * @return
	...
	/**
	 * Reduces vb = x.y; (1) x.y = vb + 1; (2) ... expr(vb); to incremental form
	 * expr(x.y++); Likewise decrement.
	 */
	...
	/**
	 * Reduces vb = x.y op z; (3) x.y = vb; (4) ... expr(vb); to incremental
	 * form expr(x.y op= z) or expr(++x.y); Likewise decrement.
	 */

...
public class Project implements Serializable {
    ...
    private Project(J2JSCompiler compiler) {
    ...
    public static Project createSingleton(File cacheFile,
        ...
        if (cacheFile != null && cacheFile.exists()) {
            ...
            try {
                ...
                Log.getLogger()
                        .warn("Could not read cache:\n" + e.getMessage());
            }
        }

        if (singleton == null
                || singleton.compressed != compiler.isCompression()
                || singleton.generateLineNumbers != compiler
                        .isGenerateLineNumbers()) {
            // Cache does not exist, could not be read, or compression does not
            // match.
            singleton = new Project(compiler);
            singleton.clear();
        }
    ...
    public Signature getArraySignature(Type type) {
         ...
         */
        if (!signatureString.startsWith("L")
                || !signatureString.endsWith(";")) {
            throw new RuntimeException(
                    "Not a class signature: " + signatureString);
        }
        signatureString = signatureString.substring(1,
                signatureString.length() - 1);
    ...
    private int getUniqueId() {
        ...
        while (currentIndex < ids.size()
                && ids.get(currentIndex) == currentId) {
            currentId += 1;
            currentIndex += 1;
        }
    ...
    private void clear() {
        ...
        currentId = 0;
        currentIndex = 0;
        compressed = compiler.isCompression();
        generateLineNumbers = compiler.isGenerateLineNumbers();
    }
    ...
    public ClassUnit getOrCreateClassUnit(String className) {
        ...
        classUnit = new ClassUnit(this, compiler.fileManager, signature);
    ...
    private MemberUnit getMemberUnitOrNull(String className,
            Signature signature) {
    ...
    private MemberUnit getMemberUnit(String className, Signature signature) {
        ...
        if (unit == null) {
            throw new RuntimeException(
                    "No such unit: " + className + "#" + signature);
        }
    ...
    public ProcedureUnit getProcedureUnit(MethodBinding methodBinding) {
        Signature signature = Project.singleton
    ...
    private MemberUnit getOrCreateMemberUnit(String className,
            Signature signature) {
    ...
    public FieldUnit getOrCreateFieldUnit(ObjectType type, String name) {
        return (FieldUnit) getOrCreateMemberUnit(type.getClassName(),
                Project.singleton.getSignature(name));
    }

    public void addReference(MethodDeclaration decl, FieldAccess fa) {
        ProcedureUnit source = getOrCreateProcedureUnit(
                decl.getMethodBinding());
        source.addTarget(Project.singleton.getSignature(fa));
    }

    public void addReference(MethodDeclaration decl,
            MethodInvocation invocation) {
        ProcedureUnit source = getOrCreateProcedureUnit(
                decl.getMethodBinding());
        source.addTarget(Project.singleton
                .getSignature(invocation.getMethodBinding().toString()));
    }
    ...
    public void addReference(MethodDeclaration decl, ArrayCreation ac) {
        ...
        for (int i = 0; i < ac.getDimensions().size(); i++) {
            ...
            source.addTarget(Project.singleton.getSignature(
                    signature.toString().substring(i) + "#length"));
        }
    }
    ...
    public void resolve(ClassUnit clazz) {
        ...
        if (clazz.getName().startsWith("[")) {
            ...
            TypeDeclaration typeDecl = new TypeDeclaration(
                    new ObjectType(clazz.getName()), 0);
            typeDecl.setSuperType(Type.OBJECT);
            typeDecl.visit(compiler.generator);
        } else {
            TypeResolver resolver = new TypeResolver(compiler, this,
                    compiler.generator);

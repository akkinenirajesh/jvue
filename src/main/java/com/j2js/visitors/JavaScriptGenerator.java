...
public class JavaScriptGenerator extends Generator {
    ...
    public JavaScriptGenerator(Project theProject, J2JSCompiler compiler) {
        super(compiler);
        project = theProject;
    ...
    public void visit(TypeDeclaration theTypeDecl) {
        ...
        if (theTypeDecl.getSuperType() != null
            ...
            superType = Project.getSingleton()
                    .getSignature(theTypeDecl.getSuperType().getClassName())
                    .getCommentedId();
        }
        ...
        print(DEFINECLASS);
        print("(");
        print(Project.getSingleton().getSignature(theTypeDecl.getClassName())
                .getCommentedId());
    ...
    public void visit(MethodDeclaration method) {
        ...
        if (method.getBody() == null) {
            ...
            throw new RuntimeException("Method " + method + " with access "
                    + method.getAccess() + " may not have empty body");
        }
        ...
        Signature signature = Project.getSingleton()
                .getSignature(methodBinding.toString()).relative();
        ...
        Iterator<VariableDeclaration> iterator = method.getParameters()
                .iterator();
    ...
    public void visit_(Block block) {
        ...
        while (node != null) {
            currentNode = node;
            if (compiler.isGenerateLineNumbers()) {
                int lineNumber = currentMethodDeclaration.getLineNumberCursor()
                        .getAndMarkLineNumber(node);
                if (lineNumber != -1) {
    ...
    private void bracket(ASTNode node, InfixExpression.Operator op) {
        if ((node instanceof InfixExpression
                && ((InfixExpression) node).getOperator() == op)
                || node instanceof NumberLiteral || node instanceof NullLiteral
    ...
    public void visit(InstanceofExpression node) {
        ...
        print(", \"");
        Signature signature = Project.getSingleton()
                .getArraySignature(node.getRightOperand());
    ...
    public void visit(ClassLiteral literal) {
        MethodBinding binding = MethodBinding.lookup("java.lang.Class",
    ...
    /*
     * We have to substitute each setter and getter method by the appropriate
     * field access. According the first 3 characters and the number of
     * arguments of the method (set*(arg) for setter, get*() for getter), we can
     * deduce whether we are looking at a setter/getter, with the following
     * exceptions: NamedNodeMap.setNamedItem(arg)
     * NamedNodeMap.setNamedItemNS(arg) Element.setAttributeNode(arg)
    ...
    private boolean isW3C(MethodInvocation invocation) {
        ...
        if (methodBinding.equals("org.w3c.dom5.Element")) {
            if (name.equals("setAttributeNode")
                    || name.equals("setAttributeNodeNS"))
                return false;
        }
    ...
    private void generateScriptCode(MethodInvocation invocation) {
        ...
        if (!(args.get(0) instanceof StringLiteral)) {
            throw new RuntimeException("First argument to " + methodBinding
    ...
    private void generateArguments(MethodInvocation invocation) {
        MethodBinding methodBinding = invocation.getMethodBinding();
        Signature signature = Project.getSingleton()
                .getSignature(methodBinding.getDeclaringClass().getClassName());
        print(signature.getCommentedId());
        print(", ");
        signature = Project.getSingleton()
                .getSignature(methodBinding.getRelativeSignature());
        print(signature.getCommentedId());
    ...
    public void visit(MethodInvocation invocation) {
        ...
        if (className.startsWith("org.w3c.dom5.")
                && !className.equals("org.w3c.dom5.views.Window")) {
            ...
            if (isW3C(invocation)) {
                ...
                // if (methodBinding.equals("org.w3c.dom.html.HTMLFrameElement")
    ...
    public void visit(ClassInstanceCreation cic) {
        ...
        print(NEWINSTANCE);
        print("(");
        print(Project.getSingleton()
                .getSignature(
                        ((ObjectType) cic.getTypeBinding()).getClassName())
                .getCommentedId());
    ...
    public void visit(ArrayCreation ac) {
        if (ac.getDimensions().size() <= 0) {
            throw new RuntimeException("Expected array dimension > 0, but was"
                    + ac.getDimensions().size());
        }
        ...
        if (ac.getInitializer() != null) {
            ...
            print("j2js.newArray('");
            Signature signature = Project.getSingleton()
                    .getArraySignature(ac.getTypeBinding());
            print(signature.toString());
            print("', [");
    ...
    public static String normalizeAccess(String name) {
        if (!name.matches("\\w*")) {
    ...
    public void visit(FieldAccess fr) {
        ...
        if (expression == null) {
            ...
            print("(" + Project.getSingleton()
                    .getSignature(fr.getType().getClassName())
                    .getCommentedId());
            print(")");

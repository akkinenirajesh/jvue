...
public class VariableDeclaration extends ASTNode {
    ...
    private AnnotationEntry[] annotations = new AnnotationEntry[0];
    ...
    public static String getLocalVariableName(Method method, int slot, int pc) {
        if (false && method.getLocalVariableTable() != null) {
            ...
            LocalVariable[] table = method.getLocalVariableTable()
                    .getLocalVariableTable();
            LocalVariable lvar = null;
            for (int i = 0; i < table.length; i++) {
                lvar = table[i];
                if (lvar.getIndex() == slot && lvar.getStartPC() <= pc
                        && pc <= lvar.getStartPC() + lvar.getLength()) {
                    return lvar.getName();
                }
            }
    ...
    public VariableDeclaration(int theLocation) {
        if (theLocation != VariableDeclaration.NON_LOCAL
                && theLocation != VariableDeclaration.LOCAL_PARAMETER) {
            throw new RuntimeException(
                    "Illegal location specified: " + theLocation);
        }
